...
usage() {
  printf 'Usage: %s <region> <hub-subscription-id> <enable-soft-delete> <environment> <storage-resource-group> <storage-account-name> <app-short-name>\n' "$0" >&2
}

if (( $# != 7 )); then
  printf 'ERROR: expected 7 arguments, received %d.\n' "$#" >&2
  usage
  exit 2
fi
